...
const Scale = ({ map }: UIOverlayProps) => {
                ...
                setZoomLabels((prevLabels) => {
                    ...
                    const remainingLabels = prevLabels
                        ...
                        .map((label) => {
                            ...
                            const newPos = newLabelsData.find(
                                (l) => l.level === label.level,
                            )!.position;
                            return {
                                ...label,
                                position: newPos,
    ...
    const updateScalerUI = useCallback((newScale: number) => {
        ...
        animationFrameRef.current = requestAnimationFrame(() => {
            if (scalerRef.current) {
                scalerRef.current.style.setProperty(
                    '--scale',
                    newScale.toString(),
                );
            }
    ...
    useEffect(() => {
        ...
        const checkMapConfigInterval = setInterval(() => {
            ...
            if (
                currentMaxZoom !== lastMaxZoomRef.current ||
                currentMinZoom !== zoomBounds.min
            ) {
                console.log(
                    `Zoom bounds changed: ${zoomBounds.min}-${zoomBounds.max} -> ${currentMinZoom}-${currentMaxZoom}`,
                );
