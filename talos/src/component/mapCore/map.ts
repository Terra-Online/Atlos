...
export class MapCore {
    ...
    constructor(ele: HTMLDivElement, options?: IMapOptions) {
        ...
        this.markerLayer = new MarkerLayer(this.map, options?.onSwitchCurrentMarker);

        this.map.on('moveend', () => {
            if (!this.transforming) {
                useViewState.getState().saveViewState(this.currentRegionId, this.map);
            }
        });
    ...
    switchRegion(regionId: string) {
        ...
        const southWest = this.map.unproject([0, config.dimensions[1]], config.maxZoom);
        const northEast = this.map.unproject([config.dimensions[0], 0], config.maxZoom);
