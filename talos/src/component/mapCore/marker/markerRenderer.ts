...
export const MARKER_ICON_DICT = Object.values(MARKER_TYPE_DICT).reduce<Record<string, L.Icon | L.DivIcon>>((acc, type) => {
...
const RENDERER_DICT: Record<string, (markerData: IMarkerData, onClick?: (markerData: IMarkerData) => void) => L.Marker> = {
  ...
  "sub_icon": (markerData, onClick) => {
    ...
    layer.bindTooltip(
      ...
      {permanent: true, className: styles.customTooltip, direction: "right"}
    ).openTooltip()
    layer.addEventListener("click", (e) => {
      e.originalEvent.stopPropagation()
      // TODO 实现切换当前marker功能
...
export function getMarkerLayer(markerData: IMarkerData, onClick?: (markerData: IMarkerData) => void) {
