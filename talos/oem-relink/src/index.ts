...
const WORKER_VERSION = '20260830.01';
...
export default {
    async fetch(request: Request, env?: Env): Promise<Response> {
        const requestUrl = new URL(request.url);
        const oeaResponse = await handleOeaPackage(request, env);
        if (oeaResponse) return oeaResponse;
        ...
        const isSocialPreview = shouldServeSocialPreview(
            userAgent,
            forcePreview,
        );
                ...
                JSON.stringify(
                    {
                        ...
                        isIntelImportDebug:
                            isIntelImportDebugRequest(requestUrl),
                        redirectUrl,
                        country,
